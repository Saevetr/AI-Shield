...
const getLineRisk = (lineId: string): QueryResult => {
  const lowerLineId = lineId.toLowerCase();

  if (lowerLineId.includes("1234") || lowerLineId.includes("money") || lowerLineId.includes("vip")) {
    return {
      level: "high",
      lineId,
      score: 88,
  ...
  if (lineId.length <= 5 || lowerLineId.includes("shop")) {
    return {
      level: "medium",
      lineId,
      score: 56,
...
export default function LineQueryScreen() {
  ...
  const handleQuery = () => {
    ...
    if (!/^[A-Za-z0-9._@-]{3,}$/.test(normalizedLineId)) {
      Alert.alert("查詢失敗", "請輸入完整 LINE ID，至少 3 個字元，可包含英文、數字、底線、句點、@ 或連字號。");
      return;
    }

    const result = getLineRisk(normalizedLineId);
        ...
        <View style={styles.resultScreen}>
          ...
          <ScrollView contentContainerStyle={styles.resultContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.resultCard}>
              ...
              <Text style={styles.resultCardText}>
                系統會依照帳號命名、用戶回報、可疑邀請與近期查詢紀錄進行初步判斷。若對方要求投資、代購或私下轉帳，請提高警覺。
              </Text>
            </View>
            ...
            <View style={styles.resultCard}>
              ...
              <Text style={styles.resultCardText}>
                目前尚無完整公開留言。你可以協助通報，讓其他使用者更快辨識可疑 LINE 帳號。
              </Text>
            </View>
          </ScrollView>
          ...
          <View style={styles.actionBar}>
            <TouchableOpacity
              ...
              onPress={() => Alert.alert("已加入黑名單", "此 LINE ID 已加入黑名單示意清單。")}
              activeOpacity={0.82}
            >
      ...
      >
        ...
        <View style={styles.queryCard}>
          <View style={styles.iconCircle}>
            <Image source={require("@/assets/images/line.png")} style={styles.lineIcon} resizeMode="contain" />
          </View>
        ...
        <View style={styles.tipCard}>
          ...
          <View style={styles.tipTextGroup}>
            ...
            <Text style={styles.tipText}>陌生帳號要求加入投資群、購買點數或私下匯款時，請先確認身分。</Text>
          </View>
        </View>
          ...
          {records.length > 0 ? (
            records.map((record) => (
              <TouchableOpacity
                ...
                onPress={() => setQueryResult(getLineRisk(record.lineId))}
                activeOpacity={0.78}
              >
                <View style={styles.recordIcon}>
                  <Image source={require("@/assets/images/line.png")} style={styles.recordLineIcon} resizeMode="contain" />
                </View>
